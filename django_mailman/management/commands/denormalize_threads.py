...
class Command(BaseCommand):
    def handle(*args, **kwargs):
        thread = OrderedDict()
        for message in ListMessage.objects.order_by('date'):
        ...
        bethreaded = set()
        for message_id, parts in thread.iteritems():
            message = parts['message']
            if message.in_reply_to and message.in_reply_to in thread:
                thread[message.in_reply_to]['children'].append(parts)
                bethreaded.add(message.message_id)

        for message_id in bethreaded:
            del thread[message_id]

        with transaction.commit_on_success():
            set_threads(thread.values())
